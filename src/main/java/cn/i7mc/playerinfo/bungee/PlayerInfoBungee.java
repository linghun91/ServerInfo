...
public class PlayerInfoBungee {
    ...
    private void loadConfig() {
        try {
            ...
            // 加载配置 - 使用InputStreamReader指定UTF-8编码
     ...
     */
    public void reload() {
        logger.info(ChatColor.DARK_AQUA + "正在重新加载 PlayerInfo BungeeCord 插件...");

        // 重新加载配置
        loadConfig();

        // 重新加载认证控制器（重要：这里会重新加载passwd.yml）
        if (authEnabled && authController != null) {
            authController.reload();
            logger.info(ChatColor.DARK_AQUA + "已重新加载认证系统配置");
        }
